...
export const bulkOnboardProducts = async (req: Request, res: Response) => {
  try {
    ...
    const results = await prisma.$transaction(async (tx) => {
      // Single variant case: Create only one standalone product
      if (variants.length === 1) {
        const variant = variants[0]!;
        const finalName = variant.variantName === 'Default' 
          ? parentName 
          : `${parentName} - ${variant.variantName}`;

        const standalone = await tx.product.create({
          data: {
            storeId,
            ...
            parentProductId: null, // No parent
            variantName: null,     // Standalone product
            price: variant.price,
        ...
        return { parent: standalone, variants: [standalone] };
      }

      // Multiple variants case: Create a parent container and child variants
